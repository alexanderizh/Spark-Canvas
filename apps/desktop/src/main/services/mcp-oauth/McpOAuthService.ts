...
import {
  SparkMcpOAuthProvider,
  type McpOAuthStore,
  type SparkOAuthTokens,
...
export interface McpOAuthServerSource {
  get(id: string): McpServerLike | null | undefined
}

export function getMcpOAuthTokenService(serverId: string): string {
  return `spark-canvas-mcp-oauth:${serverId}`
}
...
export class DesktopMcpOAuthStore implements McpOAuthStore {
  ...
  async getTokens(serverId: string): Promise<SparkOAuthTokens | undefined> {
    return await this.read(serverId, 'tokens')
  }
  async saveTokens(serverId: string, tokens: SparkOAuthTokens): Promise<void> {
    await this.write(serverId, 'tokens', tokens)
  }
  async clearTokens(serverId: string): Promise<void> {
    await this.remove(serverId, 'tokens')
  }
  async getClientInformation(serverId: string): Promise<OAuthClientInformationMixed | undefined> {
    return await this.read(serverId, 'client_information')
  }
  async saveClientInformation(serverId: string, info: OAuthClientInformationMixed): Promise<void> {
    await this.write(serverId, 'client_information', info)
  }
  async clearClientInformation(serverId: string): Promise<void> {
    await this.remove(serverId, 'client_information')
  }
  async getDiscoveryState(serverId: string): Promise<OAuthDiscoveryState | undefined> {
    return await this.read(serverId, 'discovery_state')
  }
  async saveDiscoveryState(serverId: string, state: OAuthDiscoveryState): Promise<void> {
    await this.write(serverId, 'discovery_state', state)
  }
  async clearDiscoveryState(serverId: string): Promise<void> {
    await this.remove(serverId, 'discovery_state')
  }
  async clearAll(serverId: string): Promise<void> {
    await (await this.store(serverId)).clear()
  }
  ...
  private async store(serverId: string) {
    ...
    if (store == null) {
      ...
      store = new TokenStore(getMcpOAuthTokenService(serverId))
      await store.load()
  ...
  private async payload(serverId: string): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      log.warn(
        `Ignoring corrupted MCP OAuth secure payload for ${serverId}: ${err instanceof Error ? err.message : String(err)}`,
      )
      return {}
    }
  }
  ...
  private async write(serverId: string, key: string, value: unknown): Promise<void> {
    ...
    const payload = await this.payload(serverId)
    await store.save({
      token: JSON.stringify({ ...payload, [key]: JSON.stringify(value) }),
      refreshToken: 'mcp-oauth',
      userId: serverId,
    })
  }
...
export class McpOAuthService {
  ...
  async saveStaticClient(
    ...
    const existingSecret =
      existing != null && 'client_secret' in existing && typeof existing.client_secret === 'string'
        ? existing.client_secret
        : undefined
    const clientSecret =
      client.clientSecret != null && client.clientSecret.length > 0
        ? client.clientSecret
        : existingSecret
  ...
  async refreshIfNeeded(
    serverId: string,
    cfg = parseServerConfig(this.source.get(serverId)?.configJson),
  ): Promise<SparkOAuthTokens | undefined> {
    ...
    if (tokens.expires_at == null || tokens.expires_at - nowSeconds() > REFRESH_SKEW_SECONDS)
      return tokens
    ...
    try {
      ...
      const refreshed = await refreshAuthorization(serverInfo.authorizationServerUrl, {
        ...(serverInfo.authorizationServerMetadata != null
          ? { metadata: serverInfo.authorizationServerMetadata }
          : {}),
        clientInformation,
        refreshToken: tokens.refresh_token,
        ...(serverInfo.resourceMetadata?.resource != null
          ? { resource: new URL(serverInfo.resourceMetadata.resource) }
          : {}),
      })
      const saved = withExpiresAt({
        ...refreshed,
  ...
  async getAuthStatus(serverId: string): Promise<McpOAuthStatus> {
    ...
    return (await this.getAccessToken(serverId)) != null
      ? 'authorized'
      : this.failed.has(serverId)
        ? 'failed'
        : 'needs-auth'
  }
  ...
  async authorize(serverId: string): Promise<void> {
    const server = this.source.get(serverId)
    const cfg = parseServerConfig(server?.configJson)
    if (server == null || cfg?.url == null || cfg.auth?.type !== 'oauth2')
      throw new Error('MCP OAuth server is not configured')
    if (this.authorizing.has(serverId))
      throw new Error('MCP OAuth authorization is already in progress')
    if (
      cfg.auth.dcr === false &&
      !cfg.auth.clientId?.trim() &&
      (await this.store.getClientInformation(serverId)) == null
    ) {
    ...
    try {
      const provider = new SparkMcpOAuthProvider({
        ...
        ...(cfg.auth.dcr === false
          ? {
              staticClient: {
                ...(cfg.auth.clientId != null ? { clientId: cfg.auth.clientId } : {}),
                ...(cfg.auth.clientSecret != null ? { clientSecret: cfg.auth.clientSecret } : {}),
              },
            }
          : {}),
        state,
      ...
      if (tokens?.access_token == null)
        throw new Error('OAuth authorization completed without an access token')
      this.failed.delete(serverId)
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err)
      this.failed.set(serverId, message)
...
function parseServerConfig(configJson?: string): ServerConfig | null {
  ...
  try {
    return JSON.parse(configJson) as ServerConfig
  } catch {
    return null
  }
}
...
async function createLoopbackCallback(): Promise<{
  ...
  const waitForCode = new Promise<{ code: string; state?: string }>((resolve, rejectPromise) => {
    settle = resolve
  ...
  const server = createServer((req, res) => {
    ...
    if (done) {
      res.writeHead(409).end('OAuth callback already consumed')
      return
    }
    done = true
